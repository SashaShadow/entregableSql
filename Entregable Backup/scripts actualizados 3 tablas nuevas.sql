...
-- Primera tabla (tiene registros).
-- select * from batallas where mons_id = 2;

-- Vamos a borrar todos los registros de objetos que empiecen con A
-- START TRANSACTION;

-- SET @ids = (SELECT GROUP_CONCAT(bat_id) FROM batallas WHERE mons_id = 2);
-- DELETE FROM batallas WHERE FIND_IN_SET(bat_id, @ids);

-- Vemos que los registros que antes estaban se eliminaron:
-- select * from batallas where mons_id = 2;

-- Ejecutando rollback los registros vuelven a estar
-- ROLLBACK;
-- select * from batallas where mons_id = 2;

-- Si ejecutaramos commit estos registros quedarian eliminados para siempre.
-- COMMIT;

-- Segunda tabla
-- START TRANSACTION;
-- INSERT INTO habilidades (descripcion, efecto) values ('Cuchillo plegable', 'Causa 15 de daño');
-- INSERT INTO habilidades (descripcion, efecto) values ('Antorchaespada', 'Quema al enemigo con un espadazo, 30 de daño');
-- INSERT INTO habilidades (descripcion, efecto) values ('Rayo rosa', 'Causa un 10% de daño de la vida actual del enemigo')
-- INSERT INTO habilidades (descripcion, efecto) values ('Magenta aura', 'Tira un dado, si sale numero par cura un 50% de la propia hp');
-- savepoint habs1;
-- INSERT INTO habilidades (descripcion, efecto) values ('Vision nocturna', 'Dos golpes de 10');
-- INSERT INTO habilidades (descripcion, efecto) values ('Cavar', 'Aumenta la propia vida en un 25');
-- INSERT INTO habilidades (descripcion, efecto) values ('Spin', '50% de chances de causar 25 de daño, sino 15');
-- INSERT INTO habilidades (descripcion, efecto) values ('Hamon', 'Causa un 20% de daño de la hp actual del enemigo');
-- savepoint habs2;

-- Linea comentada que elimina el primer savepoint
-- RELEASE habs1;


-- NUEVAS 3 TABLAS PARA LLEGAR A LAS 15: ---------------------------------------------------------------------

CREATE TABLE tiendas(
tienda_id INT NOT NULL AUTO_INCREMENT, nombre VARCHAR(50), PRIMARY KEY (tienda_id));

INSERT INTO tiendas (nombre) values ("Geri"), ("Raion"), ("Cafe 13"), ("Mi au"), ("Oh God Store");

-- select obj_id from objetos ORDER BY RAND() LIMIT 1;
-- select tienda_id from tiendas ORDER BY RAND() LIMIT 1;

CREATE TABLE tiendas_objetos(
tienda_id INT NOT NULL, obj_id INT NOT NULL,
FOREIGN KEY (tienda_id) REFERENCES tiendas(tienda_id),
FOREIGN KEY (obj_id) REFERENCES objetos(obj_id));

-- Insertamos algunos registros al azar ejecutando esta consulta algunas veces:
INSERT INTO tiendas_objetos (tienda_id, obj_id)
SELECT t.tienda_id, o.obj_id
FROM (SELECT tienda_id FROM tiendas ORDER BY RAND() LIMIT 1) AS t, 
     (SELECT obj_id FROM objetos ORDER BY RAND() LIMIT 1) AS o;
...
-- select * from tiendas_objetos;

-- Creacion de tabla de auditoria cada vez que se crea un personaje nuevo (mas trigger)
CREATE TABLE registro_personajes(
pers_id INT NOT NULL, usuario_creador VARCHAR(50), fecha DATETIME);

DELIMITER STRS
CREATE TRIGGER auditoria_personajes
AFTER INSERT ON personajes
FOR EACH ROW
BEGIN
